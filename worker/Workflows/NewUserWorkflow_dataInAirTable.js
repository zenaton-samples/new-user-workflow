...
module.exports = workflow('NewUserWorkflow_dataInAirTable', function* (user) {
  const template_blocks = yield this.run.task('GetTemplateBlocksFromAirTable')
  ...
  if (null === installation_event) {
    ...
    yield this.run.task('SendEmailWithLocalTemplate', {
      email: user.email,
      subject: 'What are the next steps ?',
      template_file: 'welcome_with_airtable',
      template_data: { agent_installed: false, name: user.name},
      template_blocks
    })
  } else {
